 ...
 * Mark user alert as seen (set seen = true). Caller must own the alert.
 */
export async function PATCH(
  ...
  if (fetchError || !existing) {
    log.warn("User alert not found", { alertId: id });
  ...
  if (updateError) {
    log.error("User alert update failed", { error: updateError.message, alertId: id });

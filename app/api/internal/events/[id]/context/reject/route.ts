/**
 * POST /api/internal/events/[id]/context/reject
 * Sets event_context.status = 'Rejected', reviewed_by, reviewed_at. Admin/Reviewer only.
 */
...
export async function POST(
  ...
  if (updateError) {
    log.error("event_context reject failed", { error: updateError.message, eventId: id });
    return internalError(updateError.message);
  }

  log.info("Context rejected", { eventId: id });
